...
const Rating = (prop: RatingProp) => {
    ...
    const filledStars = [...Array(numSolidStars).keys()].map((i) => (
        <FontAwesomeIcon key={i} icon={['fas', 'star']} style={style} />
    ));
    const halfStar = hasHalf
        ...
        : [];
    const unfilledStars = [...Array(numRegStars).keys()].map((i) => (
        <FontAwesomeIcon key={i} icon={['far', 'star']} style={style} />
    ));
    const ratingComponents = [...filledStars, ...halfStar, ...unfilledStars];
    return <div style={{ display: 'flex' }}>{...ratingComponents}</div>;
};
